	...
	<li class="list-group-item list-group-item-<?= $mensagem->get('data_leitura')? 'success' : 'warning' ?>">
		<p class="list-group-item-heading">
			<div class="pull-right">
				<a data-toggle="tooltip" title="Abrir" class="btn btn-sm btn-success edit" href="<?= Controller::route('mensagem', 'modalReply', $mensagem->get('id'))?>">
					<i class="fa fa-envelope-open"></i>
				</a>
				<?php if (!$mensagem->get('data_leitura') && Session::getVoluntario('id') == $mensagem->get('remetente_id')): ?>			
				<a data-toggle="tooltip" title="Deletar" class="btn btn-sm btn-danger async-confirm" data-pagination="#pag-mensagens" href="<?= Controller::route('mensagem', 'delete', $mensagem->get('id'))?>">
