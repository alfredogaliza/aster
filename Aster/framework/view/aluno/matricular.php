		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<form class="form-horizontal form-async" id="form" method="post" action="<?php echo Controller::route("matricula", "gravar")?>">
					...
					<input name="inclusao" type="hidden" value="<?php echo $this->matricula->get('inclusao')?>" />
					...
					<div class="row form-group">
						...
						<div class="col-md-8">
							...
							<select id="polo-select" class="form-control">
								...
								<?= Model::getOptions('polo', 'id', 'quartel', $this->matricula->getAluno('polo_id')) ?>
							</select>
							<?php else: ?>
								<?= $this->matricula->getAluno()->getPolo('quartel') ?>
							<?php endif;?>
						</div>						
					</div>					
					<div class="row form-group">
...
<script>
	...
	$("#polo-select").change(function(){
		$("#turma-select").load("<?= Controller::route('matricula', 'optionsTurmas')?>/?polo_id="+$(this).val());
	});

	$("#turma-select").load("<?= Controller::route('matricula', 'optionsTurmas')?>/?polo_id=<?= $this->matricula->getAluno('polo_id')?>");
</script>
