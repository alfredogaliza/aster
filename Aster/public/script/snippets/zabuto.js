 ...
 * Zabuto Calendar
 * 
 * Dependencies - jQuery (2.0.3) - Twitter Bootstrap (3.0.2)
 */
...
$.fn.zabuto_calendar = function(options) {
	...
	this
			.each(function() {
				...
				$calendarElement.attr('id', "zabuto_calendar_"
				...
				function drawLegend($calendarElement) {
					var $legendObj = $('<div class="legend" id="'
							+ $calendarElement.attr('id') + '_legend"></div>');
												...
												if ('type' in item) {
													...
													case 'block':
														...
														if (typeof (item.classname) === 'undefined') {
															...
															var listClassName = 'event-styled '
																	+ item.classname;
														}
														$legendObj
																.append('<span class="legend-'
					...
					for (var wk = 0; wk < weeksInMonth; wk++) {
						...
						for (var dow = 0; dow < 7; dow++) {
							if (dow < firstDow
								...
								var dateId = $calendarElement.attr('id')
										+ '_'
										+ dateAsString(year, month,
												currDayOfMonth);
								var dayId = dateId + '_day';

								var $dayElement = $('<div id="' + dayId
										+ '" class="day" >' + currDayOfMonth
										+ '</div>');
				...
				function drawEvents($calendarElement, type) {
					...
					if (events !== false) {
						$(events).each(function(index, value) {
											...
											if (typeof (value.popover) !== 'undefined'
												&& value.popover !== false) {													
													$dayElement
...
$.fn.zabuto_calendar_language = function(lang) {
	...
	case 'pt_br':
		return {
			month_labels : [ "Janeiro", "Fevereiro", "Março", "Abril", "Maio",
